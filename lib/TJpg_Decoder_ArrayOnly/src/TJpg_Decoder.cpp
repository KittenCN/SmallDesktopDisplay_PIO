...
unsigned int TJpg_Decoder::jd_input(JDEC* jdec, uint8_t* buf, unsigned int len)
{
  ...
  // Avoid running off end of the PROGMEM array.
  const uint32_t remaining = thisPtr->array_index < thisPtr->array_size
                                 ? thisPtr->array_size - thisPtr->array_index
                                 : 0;
  if (len > remaining) len = remaining;

  if (buf) memcpy_P(buf, thisPtr->array_data + thisPtr->array_index, len);
