...
def parse_vlw(data: bytes) -> tuple[bytes, list[tuple[int, bytes, bytes]], bytes]:
    """Parse the subset of Processing VLW used by TFT_eSPI smooth fonts."""

    if len(data) < 24:
        raise ValueError("VLW 数据短于 24 字节 header")
    glyph_count = struct.unpack(">I", data[:4])[0]
    if glyph_count == 0:
        raise ValueError("VLW 字体没有字形")
    if glyph_count > 0xFFFF:
        raise ValueError("VLW 字形数超出 TFT_eSPI uint16_t 上限")
    metrics_end = 24 + glyph_count * 28
    if metrics_end > len(data):
        raise ValueError("VLW metrics 表被截断")

    bitmap_offset = metrics_end
    glyphs: list[tuple[int, bytes, bytes]] = []
    seen_codepoints: set[int] = set()
    for index in range(glyph_count):
        record_start = 24 + index * 28
        record = data[record_start : record_start + 28]
        codepoint, height, width, advance, _, _, _ = struct.unpack(">7I", record)
        delta_y = struct.unpack(">i", record[16:20])[0]
        delta_x = struct.unpack(">i", record[20:24])[0]
        if codepoint > 0xFFFF or codepoint in seen_codepoints:
            raise ValueError(f"VLW 字形码点无效或重复: U+{codepoint:04X}")
        if height > 0xFF or width > 0xFF or advance > 0xFF:
            raise ValueError(f"VLW 字形 U+{codepoint:04X} 尺寸超出 uint8_t")
        if not -0x8000 <= delta_y <= 0x7FFF or not -0x80 <= delta_x <= 0x7F:
            raise ValueError(f"VLW 字形 U+{codepoint:04X} 偏移超出 TFT_eSPI 范围")
        seen_codepoints.add(codepoint)
        bitmap_size = width * height
        bitmap_end = bitmap_offset + bitmap_size
        if bitmap_end > len(data):
            raise ValueError(f"VLW 字形 U+{codepoint:04X} bitmap 被截断")
        glyphs.append((codepoint, record, data[bitmap_offset:bitmap_end]))
        bitmap_offset = bitmap_end

    return data[4:24], glyphs, data[bitmap_offset:]


def subset_vlw(data: bytes, characters: str) -> bytes:
    """Return a VLW containing only requested glyphs, without rerasterizing."""

    requested = list(
        dict.fromkeys(ord(char) for char in characters if char not in " \t\r\n")
    )
    if not requested:
        raise ValueError("字符清单为空")
    header_tail, glyphs, footer = parse_vlw(data)
    requested_set = set(requested)
    available = {codepoint for codepoint, _, _ in glyphs}
    missing = [codepoint for codepoint in requested if codepoint not in available]
    if missing:
        missing_text = "".join(chr(codepoint) for codepoint in missing)
        raise ValueError(f"VLW 源字体缺少请求字形: {missing_text!r}")

    selected = [glyph for glyph in glyphs if glyph[0] in requested_set]
    return b"".join(
        [struct.pack(">I", len(selected)), header_tail]
        + [record for _, record, _ in selected]
        + [bitmap for _, _, bitmap in selected]
        + [footer]
    )


def write_c_header(path: str, data: bytes, array_name: str) -> None:
    if _C_IDENTIFIER_RE.fullmatch(array_name) is None:
        raise ValueError(f"name 不是有效的 C 标识符: {array_name!r}")
    output = _ensure_parent(path)
    with output.open("w", encoding="utf-8", newline="\n") as stream:
        stream.write("// Generated by font_translate.py; do not edit manually.\n")
        stream.write("#pragma once\n")
        stream.write("#include <stdint.h>\n")
        stream.write("#include <pgmspace.h>\n")
        stream.write(f"const uint8_t {array_name}[] PROGMEM = {{\n")
        tokens = [f"0x{byte:02X}" for byte in data]
        for index in range(0, len(tokens), 16):
            stream.write("    " + ", ".join(tokens[index : index + 16]) + ",\n")
        stream.write("};\n")


def subset_vlw_header(
    input_header: str,
    characters_path: str,
    output_header: str,
    array_name: str,
) -> None:
    source_data = parse_c_header_array(input_header, array_name=array_name)
    characters = Path(characters_path).read_text(encoding="utf-8")
    compact_data = subset_vlw(source_data, characters)
    write_c_header(output_header, compact_data, array_name)
    original_count = struct.unpack(">I", source_data[:4])[0]
    compact_count = struct.unpack(">I", compact_data[:4])[0]
    print(
        f"Subset VLW {original_count} -> {compact_count} glyphs, "
        f"{len(source_data)} -> {len(compact_data)} bytes; wrote {output_header}"
    )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    subset_vlw_parser = subcommands.add_parser(
        "subset-vlw", help="Subset a TFT_eSPI/Processing VLW header without rerasterizing"
    )
    subset_vlw_parser.add_argument("--input", required=True, help="source C header")
    subset_vlw_parser.add_argument("--chars", required=True, help="UTF-8 character list")
    subset_vlw_parser.add_argument("--out", required=True, help="output C header")
    subset_vlw_parser.add_argument("--array", required=True, help="source/output array name")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        elif args.cmd == "subset-vlw":
            subset_vlw_header(args.input, args.chars, args.out, args.array)
